...
public class AuthService {
    ...
    public AuthenticationResponse authenticateEmployee(AuthenticationRequest request) {
        ...
        Optional<Employee> user = employeeRepository.findEmployeeByUsername(request.getUsername());
        if (user.isEmpty()) {
            return AuthenticationResponse.builder().message("Wrong username or password").build();
        } else {
            if (request.getPassword().equals(user.get().getPassword())) {
                var jwtToken = jwtService.generateToken(user.get());
                return AuthenticationResponse.builder().token(jwtToken).employee(user.get())
                        .message("User Logged In Successfully").build();
            } else {
                return AuthenticationResponse.builder().message("Wrong username or password").build();
            }
